...
export default function App(){
  ...
  const gen=async()=>{
    ...
    try{
      const { createClient } = await import("@fal-ai/client");
      ...
      <div style={{maxWidth:1200,margin:'0 auto'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1.2fr',gap:16,marginTop:20}}>
          <div style={{background:'#111113',border:'1px solid #222',borderRadius:16,padding:16}}>
            ...
            <button onClick={gen} disabled={loading} style={{width:'100%',marginTop:12,height:48,borderRadius:12,border:'none',background:'linear-gradient(90deg,#7c3aed,#ec4899)',color:'#fff',fontWeight:800,cursor:'pointer'}}>
          ...
          <div style={{background:'#111113',border:'1px solid #222',borderRadius:16,height:420,display:'grid',placeItems:'center',overflow:'hidden'}}>
            {video?<video src={video} controls autoPlay loop style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{textAlign:'center',opacity:0.5}}><div style={{fontSize:50}}>🎥</div><div style={{marginTop:8}}>Super Design - Video Ikada Vasthundi</div><div style={{fontSize:11,marginTop:6,opacity:0.5}}>Prompt enter chesi Generate kottu</div></div>}
          </div>
        </div>
      </div>
    </div>
