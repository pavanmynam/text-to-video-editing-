...
const CHIPS = [
  ...
  {l:"🌃 Cyberpunk", p:"cyberpunk Hyderabad neon wet streets 4k"},
  {l:"🏞️ Anime Lake", p:"Hussain Sagar anime ghibli cherry blossoms"},
  {l:"🚁 Drone", p:"cinematic drone mountains fog sunrise 8k"},
...
export default function App(){
  ...
  const [status,setStatus]=useState("Ready ✅ $8.60 • cheap $0.15/video");
  const [loading,setLoading]=useState(false);
  const [history,setHistory]=useState([]);

  const gen=async()=>{
    if(!prompt.trim()) return;
    setLoading(true); setStatus("Generating 60s...");
    try{
      const r = await fal.subscribe("fal-ai/minimax-video/text-to-video",{
        inputs:{prompt},
        logs:true,
        onQueueUpdate:(u)=> setStatus(u.status+" • "+(u.logs?.slice(-1)[0]?.message||""))
      });
      const url = r.data?.video?.url || r.video?.url;
      if(url){ setVideo(url); setHistory(h=>[{url,prompt,t:Date.now()},...h].slice(0,10)); setStatus("Done ✅"); }
    ...
    <div style={{minHeight:'100vh', background:'#050507', color:'#fff', fontFamily:'Inter,system-ui', padding:12}}>
      ...
      <div style={{maxWidth:1400, margin:'0 auto'}}>
        <div style={{height:60, background:'#12121a', border:'1px solid #242436', borderRadius:18, display:'flex', justifyContent:'space-between', alignItems:'center', padding:'0 16px', marginBottom:12}}>
          ...
          <div style={{display:'flex', gap:8, fontSize:11}}><span style={{background:'#1a1a24', border:'1px solid #2a2a3a', padding:'6px 12px', borderRadius:20}}>$8.60</span><span style={{background:'#10b9811a', color:'#10b981', border:'1px solid #10b98133', padding:'6px 12px', borderRadius:20}}>Key Ready</span></div>
        </div>
        <div style={{display:'grid', gridTemplateColumns:'360px 1fr', gap:12}}>
          <div style={{background:'#12121a', border:'1px solid #242436', borderRadius:20, padding:16}}>
            ...
            <button onClick={gen} disabled={loading} style={{width:'100%', marginTop:14, height:46, borderRadius:12, border:'none', background:'linear-gradient(90deg,#7c3aed,#3b82f6)', color:'#fff', fontWeight:800, cursor:'pointer'}}>{loading?'⏳ Generating...':'✨ Generate Video'}</button>
          ...
          <div style={{background:'#12121a', border:'1px solid #242436', borderRadius:20, padding:14}}>
            ...
            <div style={{background:'#08080c', border:'1px solid #1e1e2a', borderRadius:16, height:400, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden'}}>
              {!video &&!loading && <div style={{textAlign:'center', opacity:0.4}}><div style={{fontSize:28}}>🎬</div>Your cinematic video will appear here</div>}
              {loading && <div style={{textAlign:'center'}}><div style={{width:36,height:36, border:'3px solid #222', borderTop:'3px solid #7c3aed', borderRadius:'50%', animation:'spin 1s linear infinite', margin:'0 auto'}}/><div style={{marginTop:8}}>Generating 60s...</div></div>}
              {video && <video src={video} controls autoPlay loop style={{width:'100%', height:'100%', objectFit:'cover'}}/>}
            </div>
            {history.length>0 && <div style={{display:'flex', gap:8, overflowX:'auto', marginTop:12}}>{history.map((h,i)=><div key={i} onClick={()=>setVideo(h.url)} style={{minWidth:120, background:'#0a0a0f', border:'1px solid #1e1e2a', borderRadius:10, padding:6, cursor:'pointer'}}><div style={{fontSize:10, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{h.prompt.slice(0,20)}</div></div>)}</div>}
          </div>
        </div>
      </div>
    </div>
